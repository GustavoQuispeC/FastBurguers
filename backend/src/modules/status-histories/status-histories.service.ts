...
export class StatusHistoriesService {
  constructor(
    ...
    private statusHistRepository: Repository<StatusHistory>,
  ...
  async registerStatus(idOrder: string, statusData: any) {
    ...
    if (!orderFound)
      throw new NotAcceptableException(`Orden ${idOrder} no encontrado`);

    const id = orderFound.orderDetails.id;
    const orderDetailFound = await this.orderDetailRepository.findOneBy({ id });
    if (!orderDetailFound)
      throw new UnprocessableEntityException(
        'No es posible procesar solicitud, error en la base de datos de ordenes de compra',
      );
    ...
    const hasStatus = statusFound.some(
      (element) => element.status === statusData.status,
    );
    if (hasStatus)
